...
def simulate_circuit(qc):
    ...
    if counts:
        ...
        return '0' * qc.num_qubits  # Retourne un état par défaut si aucune mesure n'est retournée
...
def generate_chords(state, base_note):
    modifiers = [0, 4, 7, 11]  # Intervalles pour un accord majeur ou mineur
    chords = []
    for i, bit in enumerate(state):
        if bit == '1':
            chords.append(base_note + modifiers[i % len(modifiers)])
    return chords
...
note_duration_ticks = int(ticks_per_beat / 3)  # Une note par tiers de temps

# Paramètres
num_qubits = 4
steps = 10
duration = 15  # Durée totale en secondes
notes_per_second = 3
...
def convert_pdf_to_image(pdf_path, image_folder, image_name="quantum_music_harmonic_sheet"):
    try:
        ...
        for i in range(len(doc)):
            page = doc.load_page(i)  # Charge la page i du PDF
            pix = page.get_pixmap(dpi=300)  # Crée une image de la page avec une résolution de 300 DPI

            # Définir le chemin pour sauvegarder l'image
            image_path = os.path.join(image_folder, f"{image_name}_{i + 1}.png")
            pix.save(image_path)  # Sauvegarde l'image en format PNG
...
pdf_path = 'quantum_music_harmonic_sheet.pdf'  # Chemin vers votre fichier PDF
image_folder = 'static/picture'  # Dossier de destination pour les images
